...
public class WXPayRequests {
  ...
  private String requestOnce(
    ...
    httpPost.addHeader(
        "User-Agent", "wxpay sdk java v1.0 " + config.getMchID()); // 很重要，用来检测 sdk 的使用情况，要不要加上商户信息？
  ...
  private String request(
    ...
    try {
      String result =
          requestOnce(
              domainInfo.domain, urlSuffix, uuid, data, connectTimeoutMs, readTimeoutMs, useCert);
      elapsedTimeMillis = WXPayUtil.getCurrentTimestampMs() - startTimestampMs;
      config.getWXPayDomain().report(domainInfo.domain, elapsedTimeMillis, null);
      WXPayReport.getInstance(config)
          .report(
              uuid,
              elapsedTimeMillis,
              domainInfo.domain,
