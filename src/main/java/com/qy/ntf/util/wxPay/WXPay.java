...
public class WXPay {
  ...
  private boolean useSandbox; // 是否使用沙盒
  private String notifyUrl;
  private WXPayRequests wxPayRequest;
  ...
  public WXPay(
    ...
    this.wxPayRequest = new WXPayRequests(config);
  }
  ...
  private void checkWXPayConfig() throws Exception {
    ...
    //        if (this.config.getPrimaryDomain() == null ||
    // this.config.getPrimaryDomain().trim().length() == 0) {
    //            throw new Exception("primary domain in config is empty");
    //        }
    //
    //        //  海外就填两个相同的？ 下面的逻辑待考虑
    //        if (this.config.getAlternateDomain() == null ||
    // this.config.getAlternateDomain().trim().length() == 0) {
   ...
   * @throws Exception
   */
  public String requestWithoutCert(
      String urlSuffix, Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs)
      throws Exception {
    String msgUUID = reqData.get("nonce_str");
    String reqBody = WXPayUtil.mapToXml(reqData);

    String resp =
        this.wxPayRequest.requestWithoutCert(
            urlSuffix, msgUUID, reqBody, connectTimeoutMs, readTimeoutMs, autoReport);
    return resp;
  }
  ...
  public String requestWithCert(
    ...
    String resp =
        this.wxPayRequest.requestWithCert(
            urlSuffix, msgUUID, reqBody, connectTimeoutMs, readTimeoutMs, this.autoReport);
    return resp;
  }
  ...
  public Map<String, String> unifiedOrder(Map<String, String> reqData) throws Exception {
    return this.unifiedOrder(
        reqData, config.getHttpConnectTimeoutMs(), this.config.getHttpReadTimeoutMs());
  }
  ...
  public Map<String, String> unifiedOrder(
      Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
    String url;
    if (this.useSandbox) {
    ...
    String respXml =
        this.requestWithoutCert(
            url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
  ...
  public Map<String, String> reverse(
    ...
    String respXml =
        this.requestWithCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
    return this.processResponseXml(respXml);
  }
  ...
  public Map<String, String> refund(Map<String, String> reqData) throws Exception {
    return this.refund(
        reqData, this.config.getHttpConnectTimeoutMs(), this.config.getHttpReadTimeoutMs());
  }
  ...
  public Map<String, String> downloadBill(
    ...
    String respStr =
        this.requestWithoutCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs)
            .trim();
   ...
   * @throws Exception
   */
  public Map<String, String> authCodeToOpenid(
      Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
    String url;
    if (this.useSandbox) {
