...
public class H5OpenacctDemo {
  public static void main(String[] args) {
    //        innerUser("15140088204","http://api.wgn9.cn/nftApi/sysUserAdmin/userOpenNotify");
    // enterprise();
  }

  /** 个人用户（页面）开户 Demo */
  public static OpenacctApplyResult innerUser(String userId) {
    OpenacctApplyParams params = new OpenacctApplyParams();
    String timestamp = LLianPayDateUtils.getTimestamp();
    params.setTimestamp(timestamp);
    params.setOid_partner(LLianPayConstant.OidPartner);
    params.setUser_id(userId);
    params.setTxn_seqno(UUID.randomUUID().toString());
    ...
    params.setNotify_url("http://api.fmcangping.com/nftApi/sysUserAdmin/userOpenNotify");
    /*
    用户类型。
    INNERUSER：个人用户
    INNERCOMPANY：企业用户
    ...
    OpenacctApplyResult openacctApplyResult =
        JSON.parseObject(resultJsonStr, OpenacctApplyResult.class);
