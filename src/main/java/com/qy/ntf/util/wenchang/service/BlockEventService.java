...
import com.qy.ntf.util.wenchang.config.ConfigCache;
...
import com.qy.ntf.util.wenchang.dto.BlockEventBean;
...
/**
 * @author kuan Created on 21/12/11.
 */
@Slf4j
public class BlockEventService extends BaseService {

  /**
   * Get block events and parse 1. Get block information based on block height 2. Obtain transaction
   * receipts based on transactions in blocks 3. Traverse the events in the transaction receipt and
   * parse
   *
   ...
   */
  public BlockEventBean getBlockEvent(BigInteger blockNumber)
  ...
  public ArrayList<BaseEventResponse> analyzeEventsByTxHash(String hash) throws Exception {
    ...
    for (int i = 0; i < receipt.getLogs().size(); i++) {
      ...
      if (ConfigCache.get().getAuthorityLogicAddress().equalsIgnoreCase(log.getAddress())) {
        ...
        } else if (log.getTopics()
            .get(0)
            .equals(EventEncoder.encode(Authority.UPDATEACCOUNTSTATE_EVENT))) {
          list.addAll(authority.getUpdateAccountStateEvents(receipt));
        } else if (log.getTopics()
            .get(0)
            .equals(EventEncoder.encode(Authority.CROSSPLATFORMAPPROVAL_EVENT))) {
        ...
        if (log.getTopics().get(0).equals(EventEncoder.encode(DDC721.TRANSFER_EVENT))) {
          list.addAll(ddc721.getTransferEvents(receipt));
            ...
            .get(0)
            .equals(EventEncoder.encode(DDC721.ENTERBLACKLIST_EVENT))) {
        ...
        if (log.getTopics().get(0).equals(EventEncoder.encode(DDC1155.TRANSFERBATCH_EVENT))) {
          list.addAll(ddc1155.getTransferBatchEvents(receipt));
            ...
            .get(0)
            .equals(EventEncoder.encode(DDC1155.TRANSFERSINGLE_EVENT))) {
          list.addAll(ddc1155.getTransferSingleEvents(receipt));
        } else if (log.getTopics()
            .get(0)
            .equals(EventEncoder.encode(DDC1155.ENTERBLACKLIST_EVENT))) {
          list.addAll(ddc1155.getEnterBlacklistEvents(receipt));
        } else if (log.getTopics()
            .get(0)
            .equals(EventEncoder.encode(DDC1155.EXITBLACKLIST_EVENT))) {
