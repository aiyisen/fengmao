...
import com.qy.ntf.util.wenchang.contract.Charge;
...
import com.qy.ntf.util.wenchang.util.HexUtils;
...
public class ChargeService extends BaseService {
  ...
  /**
   * The operator and the platform party call this interface to recharge the account of the same
   * level or the lower-level account of the same party.
   *
  ...
  public String recharge(String sender, String to, BigInteger amount) throws Exception {
    ...
    return signAndSend(charge, Charge.FUNC_RECHARGE, encodedFunction, signEventListener, sender)
        .getTransactionHash();
  }

  /**
   * The operator, platform or end user can call the method to query the balance of the specified
   * account.
   *
  ...
  /**
   * The operator, the platform party or the end user can query the invocation service fee
   * corresponding to the method of the specified DDC service contract by invoking this method.
   *
  ...
  public String selfRecharge(String sender, BigInteger amount) throws Exception {
    ...
    return signAndSend(charge, Charge.FUNC_SELFRECHARGE, encodedFunction, signEventListener, sender)
        .getTransactionHash();
  }

  /**
   * The operator can set the method call fee of the specified DDC main contract by calling this
   * method.
   *
   * @param sender Caller address
   * @param ddcAddr DDC business contract address
   ...
   * @throws Exception
   */
  public String setFee(String sender, String ddcAddr, String sig, BigInteger amount)
      throws Exception {
    if (!AddressUtils.isValidAddress(sender)) {
      throw new DDCException(ErrorMessage.SENDER_ACCOUNT_IS_NOT_ADDRESS_FORMAT);
    }
    ...
    return signAndSend(charge, Charge.FUNC_SETFEE, encodedFunction, signEventListener, sender)
        .getTransactionHash();
  }

  /**
   * The operator can delete the method call fee of the specified DDC main contract by calling this
   * method.
   *
   * @param sender Caller address
   * @param ddcAddr DDC business contract address
  ...
  public String delFee(String sender, String ddcAddr, String sig) throws Exception {
    ...
    return signAndSend(charge, Charge.FUNC_DELFEE, encodedFunction, signEventListener, sender)
        .getTransactionHash();
  }

  /**
   * The operator can delete the specified DDC business contract authorization by calling this
   * method.
   *
   * @param sender Caller address
   * @param ddcAddr DDC business contract address
  ...
  public String delDDC(String sender, String ddcAddr) throws Exception {
    ...
    return signAndSend(charge, Charge.FUNC_DELDDC, encodedFunction, signEventListener, sender)
        .getTransactionHash();
  }
}
