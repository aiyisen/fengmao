...
import com.qy.ntf.util.wenchang.dto.Account;
import com.qy.ntf.util.wenchang.dto.TxInfo;
import com.qy.ntf.util.wenchang.exception.DDCException;
import com.qy.ntf.util.wenchang.listener.SignEvent;
import com.qy.ntf.util.wenchang.listener.SignEventListener;
import com.qy.ntf.util.wenchang.util.Bech32Utils;
import com.qy.ntf.util.wenchang.util.CommonUtils;
import com.qy.ntf.util.wenchang.util.GasProvider;
...
public class BaseService {
  ...
  public EthBlock.Block getBlockByNumber(BigInteger blockNumber) throws IOException {
    return Web3jUtils.getWeb3j()
        .ethGetBlockByNumber(CommonUtils.getDefaultBlockParamter(blockNumber.toString()), true)
        .send()
        .getBlock();
  }
   ...
   */
  public TransactionReceipt getTransReceipt(String hash)
  ...
  public TxInfo getTransByHash(String hash) throws IOException {
    Transaction transaction =
        Web3jUtils.getWeb3j().ethGetTransactionByHash(hash).send().getTransaction().get();
   ...
   * @throws ExecutionException
   */
  public EthSendTransaction signAndSend(
      Contract contract,
      String functionName,
    ...
    EthGetTransactionCount ethGetTransactionCount =
        web3j.ethGetTransactionCount(sender, DefaultBlockParameterName.PENDING).sendAsync().get();
    BigInteger nonce = ethGetTransactionCount.getTransactionCount();

    // 生成待签名的交易
    RawTransaction rawTransaction =
        RawTransaction.createTransaction(nonce, gasPrice, gasLimit, contractAddr, encodedFunction);
  ...
  public Account createAccountHex() {
    ...
    DeterministicKey deterministicKey =
        deterministicHierarchy.deriveChild(
            BIP44_ETH_ACCOUNT_ZERO_PATH, false, true, new ChildNumber(0));
