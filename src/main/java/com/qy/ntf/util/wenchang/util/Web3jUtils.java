...
public class Web3jUtils {
  ...
  public static Authority getAuthority() {
    if (authority == null) {
      authority =
          Authority.load(
              ConfigCache.get().getAuthorityLogicAddress(),
              Web3jUtils.getWeb3j(),
  ...
  public static Charge getCharge() {
    if (charge == null) {
      charge =
          Charge.load(
              ConfigCache.get().getChargeLogicAddress(),
              Web3jUtils.getWeb3j(),
              Credentials.create(ecKeyPair),
              new GasProvider());
    }
  ...
  public static DDC721 getDDC721() {
    if (ddc721 == null) {
      ddc721 =
          DDC721.load(
              ConfigCache.get().getDdc721Address(),
              Web3jUtils.getWeb3j(),
              Credentials.create(ecKeyPair),
              new GasProvider());
    }
  ...
  public static DDC1155 getDDC1155() {
    if (ddc1155 == null) {
      ddc1155 =
          DDC1155.load(
              ConfigCache.get().getDdc1155Address(),
              Web3jUtils.getWeb3j(),
              Credentials.create(ecKeyPair),
              new GasProvider());
    }
  ...
  public static Web3j getWeb3j() {
    ...
    if (web3j == null) {
      ...
      if (!Strings.isEmpty(ConfigCache.get().getHeaderKey())
          && !Strings.isEmpty(ConfigCache.get().getHeaderValue())) {
