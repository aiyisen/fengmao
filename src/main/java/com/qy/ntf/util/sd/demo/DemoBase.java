...
import com.qy.ntf.util.sd.moudle.*;
...
public class DemoBase {
  ...
  public static JSONObject requestServer(
      JSONObject request, String reqAddr, String transCode, String merId, String plId) {
    ...
    try {
          ...
          new String(
              Base64.encodeBase64(
                  CryptoUtil.AESEncrypt(
                      plainBytes, aesKeyBytes, "AES", "AES/ECB/PKCS5Padding", null)),
              "UTF-8");

      String sign =
          new String(
              Base64.encodeBase64(
                  CryptoUtil.digitalSign(plainBytes, CertUtil.getPrivateKey(), "SHA1WithRSA")),
              "UTF-8");
          ...
          new String(
              Base64.encodeBase64(
                  CryptoUtil.RSAEncrypt(
                      aesKeyBytes, CertUtil.getPublicKey(), 2048, 11, "RSA/ECB/PKCS1Padding")),
              "UTF-8");
      ...
      try {
        ...
        result =
            HttpClient.doPost(
                SDKConfig.getConfig().getUrl() + reqAddr, reqMap, connectTimeout, readTimeout);
      ...
      byte[] merchantAESKeyBytes =
          CryptoUtil.RSADecrypt(
              decodeBase64KeyBytes, CertUtil.getPrivateKey(), 2048, 11, "RSA/ECB/PKCS1Padding");

      byte[] decodeBase64DataBytes = Base64.decodeBase64(retEncryptData.getBytes("UTF-8"));

      byte[] respDataBytes =
          CryptoUtil.AESDecrypt(
              decodeBase64DataBytes, merchantAESKeyBytes, "AES", "AES/ECB/PKCS5Padding", null);
      ...
      boolean isValid =
          CryptoUtil.verifyDigitalSign(
              respDataBytes, signBytes, CertUtil.getPublicKey(), "SHA1WithRSA");
